...
int find_best_compatible(vector<float> &src, ARMNN_P *armnn_pt)
{
...
int main(int argc, char** argv)
{
  // Initialize face detection model
    ...
    if(get_input(path,input_array,armnn_p.detector,armnn_p.point_detector)){
        return -1;
    }

    inputDataContainers[0] = {input_array}; 
    armnn::Status ret = runtime->EnqueueWorkload(armnn_p.networkIdentifier,
          armnnUtils::MakeInputTensors(armnn_p.inputBindings, inputDataContainers),
          armnnUtils::MakeOutputTensors(armnn_p.outputBindings, outputDataContainers));
    ...
    gettimeofday(&stop_time, nullptr); 
    std::cout << "time from detect to identify ms: "<< (get_us(stop_time) - get_us(start_time)) / (1000)<<"\n";
    
    

    gettimeofday(&start_time, nullptr); 
    get_input(path2,input_array,armnn_p.detector,armnn_p.point_detector);
    ...
    ret = runtime->EnqueueWorkload(armnn_p.networkIdentifier,
          armnnUtils::MakeInputTensors(armnn_p.inputBindings, inputDataContainers),
          armnnUtils::MakeOutputTensors(armnn_p.outputBindings, outputDataContainers));
