...
#define GPIO_DELAY_KEY "MXFC_GPIO_DELAY"

#define OUTPUT_UART_DEV "/dev/ttyS1"
#define GPIO_DEV "/sys/devices/platform/mx_gpios/mx_gpio"
...
extern int get_capture_pic(struct V4L2_DEV *v4l2);
extern bool ReadConfig(const string & filename, map<string, string> & m);
extern int GetConfig(const map<string, string> & m, string key_name,string &key);
...
void skip_file_suffix(char *buf)
{
    int len = strlen(buf);
		...
		if( dp->d_type == DT_REG){
			if(strlen(dp -> d_name) > MAX_PATH){
                ...
                if(image_detect(name, input_array, armnn_pt, 1) == 1){
                    ...
                    skip_file_suffix(armnn_pt->feature_info[armnn_pt->feature_cnt].name);
...
void *thread_gpio(void *arg)
{
    ...
    ARMNN_P *armnn_pt = (ARMNN_P *)arg;
    int fd = armnn_pt->gpio_fd;

	printf("thread_gpio running\n");

	while(1){
        if(gpio_en){
#ifdef  HARDWARE_CHECK          
            write(fd,"1",1);
            usleep(armnn_pt->gpio_delay * 1000);
            gpio_en = 0;
            write(fd,"0",1);
#endif
        }
...
void read_config_file(ARMNN_P *armnn_pt)
{
    map<string, string> m;
    string key_val;
    int delay = -1;;
    ...
    ReadConfig(CONFIG_FILE, m);
    
    if(GetConfig(m,GPIO_DELAY_KEY,key_val) == 0)
        delay=atoi(key_val.c_str());
    else
        delay = 500;
    printf("configure file : gpio delay %dms\n",delay);
    
    armnn_pt->gpio_delay = delay;
    
}

int hardware_check(int fd)
{
    char buf[2] = {0,0};
...
// FC+name+\n
void output_result(int fd,char *buf)
{
    char out[MAX_PATH];
    if(fd > 0){
        memcpy(out,"FC+",3);
        memcpy(out+3,buf,strlen(buf));
        out[3+strlen(buf)] = '\r';
        out[4+strlen(buf)] = '\n';
        write(fd,out,5+strlen(buf));
...
int main(int argc, char** argv)
{
  // Initialize face detection model
...
#ifdef HARDWARE_CHECK   
    armnn_pt->gpio_fd = open(GPIO_DEV,O_RDWR);
    if( armnn_pt->gpio_fd < 0 ){
        printf("open gpio dev failed\n");
        return -1;
    }
    if(hardware_check(armnn_pt->gpio_fd)< 0)
        return -1;
#endif    
    ...
    if(!image_path){
        armnn_pt->uart_fd = uart_init(OUTPUT_UART_DEV);
        
        ret = pthread_create(&ntid2, NULL, thread_gpio, (void *)armnn_pt); 
        if (ret != 0){ 
            printf("can't create thread_gpio\n");	
        }
        
    }    

            
    read_config_file(armnn_pt);
    ...
    if(image_path){
        ...
        ret = image_detect(image_path, input_array, armnn_pt, MAX_FACE_NUM);
        if(ret > 0 ){
            for(i=0;i<ret;i++){
            ...
            if(ret > 0){
                for(i=0;i<ret;i++){
                    ...
                    if(index >=0 && score <= 120){
                        printf("find best compatible %s, score %f\n",armnn_pt->feature_info[index].name,score);
                        gpio_en = 1;
                        output_result(armnn_pt->uart_fd,armnn_pt->feature_info[index].name);
                    }
                }
            }    
        }
